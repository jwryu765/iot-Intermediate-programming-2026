/*
	...
	unique_ptr: 소유줜을 독점적으로 관라하는 스마트 포인터
*/
#include <iostream>
...
public:
	...
	int main() {
		std::unique_ptr<Myclass> ptr(new Myclass());		// 동적으로 생성된 Myclass 객체를 unique_ptr 포인터가 가리킨다

		std::unique_ptr<Myclass> ptr1 = std::make_unique<Myclass>();		// C++14 new 사용을 자제한다

		auto ptr2 = std::make_unique<Myclass>();

		//std::unique_ptr<Myclass> ptr3 = ptr2; 식으로 복사는 안된다. 소유권은 독점한다
